...
public class WebFetcher {
    ...
    private static List<Map<String, Object>> fetchTopRoom() {
        ...
        // 转化成有序列表
        List<String> dirList = new ArrayList<>(getDirNameSet());
        // 遍历所配的目录
        dirList.forEach(dir -> {
        ...
        for (Future<String> future : futureList) {
            ...
            for (int page = 1; page <= pageSize; page++) {
                ...
                lii.forEach(li -> {
                    ...
                    entryList.add(new AbstractMap.SimpleEntry<>(roomId, popularity));
                });
            }
        }
    ...
    private static Set<String> getDirNameSet() {
        Set<String> dirNameSet = LocalFileProperties.getDirNames();
        if (!dirNameSet.isEmpty()) {
            return dirNameSet;
        }
        ...
        Document doc;
        try {
            doc = getDocument(Constants.HTTPS_WWW_DOUYU_COM_DIRECTORY, param);
        } catch (IOException e) {
            logger.warn(e);
            return dirNameSet;
        }
        ...
        lii.forEach(li -> {
            try {
                ...
                elements.get(0).attributes().forEach(attributes -> {
                    if ("href".equals(attributes.getKey())) {
                        String[] uList = attributes.getValue().split("/");
                        dirNameSet.add(uList[uList.length - 1]);
                    }
                });
            } catch (Exception e) {
                logger.error(e);
    ...
    private static List<Map<String, Object>> getRoomMaps(List<Map.Entry<Integer, Double>> entryList) {
        ...
        entryList.forEach(entry -> tasks.add(() -> LocalCache.INSTANCE.queryFansNum(entry.getKey())));
        ...
        return WebFetcher.fetchRoomInfo(duplicate);
    }
     ...
     * 会等待全部任务执行完一起返回结果
     */
    public static List<Map<String, Object>> fetchRoomInfo(final Collection<Integer> roomIdList) {
