...
import in.odachi.douyubarragecollector.app.Consumer;
...
public class EventHandler {
    ...
    private final BlockingQueue<Map.Entry<String, LocalDateTime>> queue;

    private final Integer roomId;

    private final Map.Entry<String, Integer> addressEntry;
    ...
    // 连续接受到海量RSS消息问题
    private static final int MAX_NUMBER_OF_RSS_MSG = 64;
     ...
     * 客户端初始化
     */
    public EventHandler(Integer roomId, Map.Entry<String, Integer> addressEntry) {
        this.roomId = roomId;
        this.queue = Consumer.getInstance().getQueue();
    ...
    public void connect(SelectionKey key) throws IOException {
        ...
        if (!channel.finishConnect()) {
            ...
            ReactorManager.getInstance().closeChannel(key);
            return;
        }
    ...
    private void processData(SelectionKey key) throws IOException, InterruptedException {
        ...
        if (dataBodyStr.startsWith("type@=rss/")) {
            ...
            if (continuousRssCount >= MAX_NUMBER_OF_RSS_MSG) {
                logger.error("Received too many RSS packet in a row, channel is closing: " + toString());
                ReactorManager.getInstance().closeChannel(key, true);
                return;
            }
        ...
        // 解析其他信息
        queue.put(new AbstractMap.SimpleImmutableEntry<>(dataBodyStr, LocalDateTime.now()));
        // 数据已经接受完，继续接受新数据
        inputBuf = null;
    }
    ...
    private void loginRoom(SelectionKey key) throws IOException {
        ...
        if (!channel.isConnected() || channel.write(writeBuf) <= 0) {
            // 登陆房间失败关闭连接
    ...
    public Map.Entry<String, Integer> getAddressEntry() {
        return addressEntry;
    }

    public String toString() {
        return roomId + " (" + addressEntry.getKey() + ":" + addressEntry.getValue() + ")";
    }
}
